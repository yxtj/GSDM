...
const std::string StrategyOFG::usage(
	...
	"Usage: " + name + " <k> <theta> <obj-fun> [sd] [net] [dces] [log]\n"
...
void StrategyOFG::parseObj(const std::string & func_str)
{
	objFun.setFunc(func_str);
}

void StrategyOFG::parseDCES(const std::string & option, const std::string & minsup, const bool flag)
{
	if(option.empty()) {
...
void StrategyOFG::parseLOG(const std::string & param, const bool flag)
{
